...
def getExperiences(request,pattern):
    ...
    namewise_experiences=[] # Container to store experiences based on pattern
    users_list=User.objects.all().filter(username__icontains=pattern)
    for user in users_list: # Iterate over all the filtered experiences
    ...
    flag=[] # To keep track of bookmarked experiences to render accordingly
    for exp in experiences:
        aux=Bookmark.objects.all().filter(experience=exp).filter(user=request.user)
        if len(aux) == 0: # If not bookmarked
            flag.append(0) 
        else : # If bookmarked
            flag.append(1)
